...
namespace WebReports.ConcreteClasses
{
    public class LiveQueueXlsReporter:AbstractReporter
    {
        ...
        public void CreateReport(Queue queueInfo)
        {
            string filePath;

            if(File.Exists($@"{Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)}\LiveQueueReport\out\{DateTime.Now.ToString("dd MMMM yyyy, ddd")}.xlsx"))
            {
                filePath =
                    $@"{Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)}\LiveQueueReport\out\{
                        DateTime.Now.ToString("dd MMMM yyyy, ddd")}.xlsx";
            }
            else
            {
                filePath = $@"{Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)}\LiveQueueReport\table.xlsx";
            }
            ...
            foreach (var regSubService in queueInfo.RegSubServices)
            {
                switch (regSubService.Code)
                {
                    case "10002619923": //Прекращение регистрации ТС
                        ...
                        break;
                    case "10000466914"://Регистрация автомототранспортного средства ранее не зарегистрированного в ГИБДД
                    ...
                    case "10000593889": //Изменение регистрационных данных АМТС
                        ...
                        break;
                    case "10000593393": //Изменение регистрационных данных, связанных с изменением регистрационных данных собственника ТС
                        SetServiceStateValue(7, cellNum, regSubService.State);
                        break;
                    case "10000593682": //Изменение регистрационных данных транспортных средств, связанных с выдачей свидетельств о регистрации, ПТС, регистрационных знаков транспортных средств, взамен утраченных, непригодных для пользования
                        SetServiceStateValue(8, cellNum, regSubService.State);
                        break;
                    case "10002619870": //Изменение регистрационных данных о собственнике (владельце) транспортного средства
                        SetServiceStateValue(9, cellNum, regSubService.State);
                        break;
                    case "10000594794": //Снятие с регистрационного учета автомототранспортного средства в связи с утилизацией
                        SetServiceStateValue(10, cellNum, regSubService.State);
                        break;
                    case "10000595058": //Снятие с регистрационного учета автомототранспортного средства в связи с вывозом за пределы Российской Федерации на постоянное пребывание
                        SetServiceStateValue(11, cellNum, regSubService.State);
                        break;
                }
            }

            foreach (var examSubService in queueInfo.ExamSubServices)
            {
                switch (examSubService.Code)
                {
                    case "10000467319"://Получение международного водительского удостоверения
                        ...
                        break;
                    case "10000467506"://Получение водительского удостоверения после прохождения профессиональной подготовки (переподготовки)
                        SetServiceStateValue(14, cellNum, examSubService.State);
                        break;
                    case "10000467646"://Замена водительского удостоверения в связи с истечением срока его действия
                        SetServiceStateValue(15, cellNum, examSubService.State);
                        break;
                    case "317348568"://Замена водительского удостоверения в случае изменения персональных данных его владельца, пришедшего в негодность и в случае его утраты (хищения)
